...
/*************************************************************************************/
  $scope.showAllDevicesFirst = function(vehicles,groupid)
  {
    $scope.allAvailableGroups.push(groupid);
    for (var i = 0; i < vehicles.length;i++) 
    {
        if(vehicles[i].status=="Moving")
        {
          $scope.allAvailableObjects.push(vehicles[i].id);
          $scope.selectedVehicles.push([vehicles[i].uniqueId,vehicles[i].name,vehicles[i].id]);
        }        
    }
    $scope.selectedLiveVehicles   = $scope.allAvailableObjects;
    $scope.selectedLiveGroups     = $scope.allAvailableGroups; 
    $rootScope.$broadcast('vehiclesOnMap',{ deviceId: 0, device: 0, selectAll:1, selectedGroup: 0, selectedGroupStatus: 0 }); 
  }
        ...
        { 
          ...
          if(positions.length>0)
          {
            ...
            if($scope.allGroupsInitialized == false) 
            {
              ...
              $scope.allVehiclespositonsData=vehicles;
              $scope.showAllDevicesFirst(vehicles,$scope.get_org_ID());
            }
            ////////////////////////
            if(angular.isUndefined($rootScope.first_device_id))
            {
              ...
              if(vmoving.length>0)
              {
                ...
                $rootScope.first_obj_id     = vmoving.id;
                $rootScope.first_device_id  = vmoving.uniqueId;
              }else{
            ...
            ////////////////////////
            $rootScope.groups={group:{id:$scope.get_org_ID(), name:$scope.get_org_name(),vehicles:vehicles}};

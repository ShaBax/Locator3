...
/*  $rootScope.$on("vehicleDetails", function(event,args){
    ...
    $scope.geolddata($scope.auth_token)
    .success(function (data){
            ...
            for(var i = 0; i < vehicles.length; i++) 
            {
              for(var j = 0; j < positions.length; j++) 
              {
                if(positions[j].deviceId==vehicles[i].id)
                {
                    ...
                    vehicles[i].position.speed  = positions[j].speed.toFixed(2);
                    vehicles[i].lastupdated     = $filter('date')(positions[j].deviceTime, 'dd/MM/yyyy HH:mm:ss');
                    vehicles[i].groupid         = $scope.get_org_ID();
                    vehicles[i].ignition_status = 0;
                    if(positions[j].attributes.di1==1 && positions[j].attributes.motion==true && positions[j].attributes.ignition==true)
                    {
                      $rootScope.online++;
...
/*                $scope.parkingMarkers = [];
              ...
              /**************************************************************************/
               ...
               // var lat =vehicles[i-1].latitude;
               // var lng = vehicles[i-1].longitude;
               // var loc = new google.maps.LatLng(lat, lng);
                //$scope.center=loc;
               // vm.map.setZoom(8);
            $rootScope.allVehiclespositonsData={group:{id:$scope.get_org_ID(), name:$scope.get_org_name(),vehicles:vehicles[i]}};
  ...
  function startTimer() {
    $scope.update_vehicles_status();
    myInterval = window.setInterval(timerHandler,10000);
   }
  function stopTimer() {
    window.clearInterval(myInterval);
   }

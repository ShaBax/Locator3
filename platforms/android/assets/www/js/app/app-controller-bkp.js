...
/*app.controller('AppCtrl', function ($scope, $state) {
  ...
  $scope.alertLeft = function(){
    showSidemenu();
  }
  $scope.hideSidebar = function(){
    hideSideMenu();
  }
  $scope.showSideMainMenu = function(){
    showSidemenu();
  }
})*/
...
app.controller('VehiclesCtrl', function ($scope,$stateParams,$state,Objects,$filter,$rootScope,$interval) {
  ...
  $scope.checkAllVehicles = function(groupId,selectedGroup){
    ...
    if (groupId) {
      ...
      /*Objects.getVehiclesList($scope.authtoken)
      .success(function (data){
        for (var i = 0; i < data.orgGroups.length; i++) {
          if(data.orgGroups[i].group.obj_group_id == selectedGroup){
            for (var j = 0; j < data.orgGroups[i].vehicles.ran.length; j++) {
              if($scope.selectedLiveVehicles.indexOf(data.orgGroups[i].vehicles.ran[j].obj_id)==-1){
                $scope.selectedVehicles.push([data.orgGroups[i].vehicles.ran[j].obj_device_id,data.orgGroups[i].vehicles.ran[j].obj_name,data.orgGroups[i].vehicles.ran[j].obj_id]); 
                $scope.selectedLiveVehicles.push(data.orgGroups[i].vehicles.ran[j].obj_id); 
                $scope.selectedVehicles.push([data.orgGroups[i].vehicles.ran[j].obj_device_id,data.orgGroups[i].vehicles.ran[j].obj_name,data.orgGroups[i].vehicles.ran[j].obj_id]);  
              }
            }          
          }
      ...
      $scope.goToNextPage(0,0,0,selectedGroup,groupId);
        ...
        for (var i = 0; i < data.orgGroups.length; i++) {
          if(data.orgGroups[i].group.obj_group_id == selectedGroup){
            for (var j = 0; j < data.orgGroups[i].vehicles.ran.length; j++) {
              ...
              if($scope.selectedLiveVehicles.indexOf(data.orgGroups[i].vehicles.ran[j].obj_id) != -1){
                var index = $scope.selectedLiveVehicles.indexOf(data.orgGroups[i].vehicles.ran[j].obj_id);
                $scope.selectedLiveVehicles.splice(index,1); 
              }

              for(var k=0;k<($scope.selectedVehicles.length);k++){           
                if($scope.selectedVehicles[k][0] == data.orgGroups[k].vehicles.ran[k].obj_device_id)
  ...
  $scope.goToNextPage = function(objId,objDeviceId,objName,selectedGroup,selectedGroupStatus){
    
    if ($scope.selectedPage == 'vehicles') {
      $state.go('live',{objId:objId,objDeviceId:objDeviceId,tripId:null,selectedGroup:selectedGroup,selectedGroupStatus:selectedGroupStatus});
...
app.controller('TripsCtrl', function ($scope,$stateParams,$state,$filter,Objects,$rootScope) {
  ...
  //$scope.historyVehIDs.push([$stateParams.objName,$stateParams.objDeviceId,$stateParams.objId]);
 ...
 // alert($scope.tripDetails.from);
  ...
  $scope.showCurrentTrack = function(deviceId,tripId){
    $state.go('live',{objId:'',objDeviceId:deviceId,tripId:tripId,selectedGroup:selectedGroup,selectedGroupStatus:selectedGroupStatus});
  }
...
app.controller('LiveCtrl', function ($scope,$stateParams,$state,NgMap,$interval,$http,$rootScope,Objects) {
  ...
  $scope.selectedGroupStatus = $stateParams.selectedGroupStatus;
  $scope.selectedGroup = $stateParams.selectedGroup;
  $scope.objTripId = '';
  $scope.tempMarker = {};
  $scope.onlineMarkers = [];
  $scope.iconFolder = '';
...
/*  var trafficLayer = new google.maps.TrafficLayer();
  ...
  $scope.getVehicleDetails = function(center){
    ...
    .success(function (data){
      ...
      if ($scope.selectedGroupStatus) {
            for (var i = 0; i < $scope.vehDetails.length; i++) {
              if ($scope.vehDetails[i].obj_gp_id == $scope.selectedGroup) { 
  ...
  $scope.getCurrentLocation = function(e,marker){  
    ...
    '<h1 style="padding: 2px; height: 33px;">'+$rootScope.infoMarker.obj_name+'</h1>'+'<a>'+'<IMG style="position: absolute; right: 9px; border: 1px solid rgb(204, 204, 204); padding: 4px; border-radius: 50%; width: 79px;" SRC="http://mylocatorplus.com/final/assets/drivers/'+$rootScope.infoMarker.driver_image+'"></a>'+

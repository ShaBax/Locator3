...
/**********************FSZ*******************************/
  $rootScope.$on("RouteDetails", function(event,args){

    $rootScope.playandpause = true;
    $scope.deviceId = args.tripsdata.deviceId;
    $scope.showCurrentRoute(args.tripsdata);
    $("#slider").slider({
      slide: function( event, ui ) {              
        vm.setTrackerPosition(ui.value);
      }
  ...
  $scope.showCurrentRoute = function(tripsdata)
  {
    if($rootScope.showMapOfTripFlag==false)
      $rootScope.showLoading();
    ...
    $scope.save_deviceID(tripsdata.deviceID);

    if ($scope.get_deviceID() != -1 || 1==1) 
    {

      Online.getCurrentRoute(tripsdata,$scope.auth_token).success(function(data)
      {
        $rootScope.hideLoading();
        $rootScope.tripHistory  = data.tripHistory;
        $rootScope.stops        = $rootScope.tripHistory.stops;
 ...
 // $scope.getGroups();
  ...
  $scope.getDetails = function(option){
    ...
    if (option == 'today') {
      ...
      Online.showVehicle(option,$scope.objID,$scope.objDeviceID,$scope.from,$scope.to,$scope.auth_token)
      .success(function (data){
  ...
  vm.getCurrentLocation = function(e,marker){  
   ...
   //$rootScope.pointerLocationAddress='545';
    ...
    $scope.vehicleId = marker.vehicle.uniqueId;
    $scope.authtoken = $scope.get_auth_token();
   // $rootScope.pointerLocationAddress='Loading...';
    $rootScope.infoMarker = marker.vehicle;

     
     
    //Code to find the current address for the popup - START

/*          var latlng = new google.maps.LatLng($rootScope.infoMarker.latitude,$rootScope.infoMarker.longitude);
          var geocoder = geocoder = new google.maps.Geocoder();
    ...
    content = '<div class="popbox_main">'+
     '<h1 style="padding: 2px; height: 33px;">'+$rootScope.infoMarker.name.toString().substr(0, 20)+'</h1>';

    if($rootScope.infoMarker.driverUniqueId>0)
      content += '<a>'+'<IMG style="position: absolute; right: 9px; border: 1px solid rgb(204, 204, 204); padding: 4px; border-radius: 50%; width: 79px;" SRC="http://mylocatorplus.net/final/assets/drivers/'+$rootScope.infoMarker.driverUniqueId+'"></a>';
    else
    ...
    content += 
     ...
     '<tr><td>Status</td><td>: '+$rootScope.infoMarker.status;

    if($rootScope.infoMarker.status=='Moving')
      content += ' at '+$rootScope.infoMarker.position.speed+' km/hr';
     
      content +='</td></tr><tr><td>Ignition</td>';

    if($rootScope.infoMarker.position.attributes.ignition==true)
      content +='<td  >: On</td>';
    else
      content +='<td  >: Off</td>';
    ...
    if ($rootScope.infoMarker.position.driver.driver_name)
      content +='<tr><td>Driver</td><td> : '+$rootScope.infoMarker.position.driver.driver_name+'</td></tr>';
    /*if($rootScope.infoMarker.position.attributes.driverUniqueId) 
      content +='<tr><td>Driver</td><td> : '+$rootScope.infoMarker.position.attributes.driverUniqueId+'</td></tr>';
    else*/ 
    if($rootScope.infoMarker.position.driver.i_button_driver_id) 
    content +='<tr><td>Driver</td><td> : '+$rootScope.infoMarker.position.driver.i_button_driver_id+'</td></tr>';
    //else 
    //  content +='<tr><td>Driver</td><td> : Not assigned</td></tr>';
    //content +='<tr><td>Driver</td><td> : '+$rootScope.infoMarker.position.attributes.driverUniqueId+'</td></tr>';
    if ($rootScope.infoMarker.position.driver.driver_phone) 
      content +='<tr><td>Phone</td><td> : '+$rootScope.infoMarker.position.driver.driver_phone+'</td></tr>';
   // else
   //   content +='<tr><td>Phone</td><td> :   </td></tr>';


    if($rootScope.infoMarker.event>0)
      content +='<tr><td>Geozone</td><td> : '+$rootScope.infoMarker.event+'</td></tr>';
    else
     ...
     content += '<tr><td style="width:21%;">Last Update</td><td> : '+$rootScope.infoMarker.lastupdated+'</td></tr>';

  

    if($rootScope.infoMarker.position.temp1 && $rootScope.infoMarker.position.attributes.temp1!=0)
     content +='<tr><td style="width:21%;">Temperature</td><td> : '+$rootScope.infoMarker.position.attributes.temp1+" Celsius"+'</td></tr>';



      content +='<tr><td colspan="2" style="font-weight:normal"><span class="iwAddress_'+$rootScope.infoMarker.uniqueId+'">'+$rootScope.infoMarker.position.address+'</span> </td></tr>'+
     '</tbody></table><div class="first_left"><div class="input-group" style="width:162px"> ';

      if($rootScope.infoMarker.position.speed=='Moving')
        content +='<i class="fa fa-power-off ignIcon" aria-hidden="true" style=" background:#47a447 "  ></i>';
      else if($rootScope.infoMarker.status=='Parking' || $rootScope.infoMarker.status=='Stopped')
        content +='<i class="fa fa-power-off ignIcon" aria-hidden="true" style="  background:#d43f3a "  ></i>';
      else if($rootScope.infoMarker.status=='Idling')
        content +=' <i class="fa fa-power-off ignIcon" aria-hidden="true" style="  background:Orange " ></i>';
      else if($rootScope.infoMarker.status=='Out of Coverage')
        content +=' <i class="fa fa-power-off  btn-gray ignIcon" aria-hidden="true" style="  background:grey "  ></i> ';

      content +=' <input type="text" style="text-align:right;background-color:white;" value="'+$rootScope.infoMarker.position.attributes.distance+'" readonly="" class="form-control iwMileage_'+$rootScope.infoMarker.uniqueId+'"   /></div></div>'+
      ' <ul  class="icon_set" >'+
      '<li ><a class="disabled"  onclick="addNotification('+$rootScope.infoMarker.uniqueId+')"  class="ttip" title="Notify Customer"><img src="assets/images/icon1.png" /></a></li>'+
      '<li><a  class="ttip"  onclick="shareLocation('+$rootScope.infoMarker.uniqueId+')" title="Share the Location"><img src="assets/images/icon3.png" /></a></li>'+
      '<li><a  class="ttip disabled  onclick="enableStreetView()" title="Street View"><img src="assets/images/icon4.png" /></a></li>'+
      '<li><a class="disabled" onclick="showCurrentTrack('+$rootScope.infoMarker.uniqueId+','+$rootScope.infoMarker.position.id+')"  class=" ttip showCurrentTrack"     title="Current Trip"><img src="assets/images/icon5.png" /></a></li>'+
...
/*    if($rootScope.infoMarker.position.attributes.driverUniqueId)
    {
          $http({
          method  : 'get',
          url     : baseUrl + 'api-v1/user/getdriverinfo',data:{driveruid:$rootScope.infoMarker.position.attributes.driverUniqueId} ,headers: {'Xtoken': $scope.authtoken}
        })
        .success(function (data){  
...
/////SHARE LOCATION--START///
  ...
  $scope.saveShareLocation = function()
  {
      ...
      $scope.authtoken = $scope.get_auth_token();
      $scope.deviceID=$rootScope.infoMarker.uniqueId;

    
      $scope.shareLocation = {
      emails: $scope.sharelocationemails,
      validto: $scope.tokenvalidity,
      deviceid: $scope.deviceID
      };

      $http({
        method  : 'post',
        url     : baseUrl + 'api-v1/user/AddNewShareLocation', headers: {'Xtoken': $scope.authtoken},data: $scope.shareLocation
      })
       .success(function (data){
  ...
  $scope.getAllShareLocations = function(deviceid){
    ...
    $http({
        method  : 'get',
        url     : baseUrl + 'api-v1/user/getAllShareLocations/'+$scope.deviceID, headers: {'Xtoken': $scope.authtoken}
    })
    .success(function (data){
  ...
  $scope.DeleteShareLocation = function(sharelocationid){
    ...
    if ($window.confirm("Are you sure you want to delete this Share Location ?")) {
      ...
      $http({
        method  : 'get',
        url     : baseUrl + 'api-v1/user/deleteShareLocation/'+sharelocationid, headers: {'Xtoken': $scope.authtoken}
      })
      .success(function (data){
        if (data.status == 'success') { 
...
/*************************************************************************************/
  $scope.update_vehicles_status = function()
  {
      Online.OnPosition().success(function(data){
        ...
        { 
            ...
            {  
              var latmax=positions[0].latitude;
              var latmin=positions[0].latitude;
              var longmax=positions[0].longitude;
            ...
            if(positions.length>0)
            {
                ...
                $scope.onlineMarkers.splice(0, $scope.onlineMarkers.length);
                angular.forEach($scope.onlineMarkers, function(marker) {
                marker.setMap(null);
            });
           ///$scope.onlineMarkers=[];
            ...
            {
              vehicles[i].position='';
              vehicles[i].status="No Connection For";
              t=i;
              for(var j = 0; j < positions.length; j++) 
              {
                if(positions[j].deviceId==vehicles[i].id)
                {
                    ...
                    if(positions[j].latitude>latmax)
                    {
                      latmax=positions[j].latitude;
                    }
                    else if(positions[j].latitude<latmin)
                    {
                      latmin=positions[j].latitude;
                    } 

                    if(positions[j].longitude>longmax)
                    {
                      longmax=positions[j].longitude;
                    }
                    else if(positions[j].longitude<longmin)
                    {
                      longmin=positions[j].longitude;
                    } 


                    vehicles[i].position        = positions[j];
                    vehicles[i].position.speed  = 2*positions[j].speed.toFixed(2);
                   // vehicles[i].lastupdated     = $filter('date')(positions[j].deviceTime, 'dd/MM/yyyy HH:mm:ss','UTC');
                    vehicles[i].lastupdated     = $filter('date')(positions[j].deviceTime, 'dd/MM/yyyy HH:mm:ss');
                    vehicles[i].groupid         = $scope.get_org_ID();
                    if(positions[j].attributes.di1==1 && positions[j].attributes.motion==true && positions[j].attributes.ignition==true || positions[j].attributes.motion==true)
                    {
                      $rootScope.online++;
                    ...
                    else if(positions[j].attributes.di1==1 && positions[j].attributes.motion==false)
                    {
                      $rootScope.idle++;
                      vehicles[i].status="Idling";
                    }
                    else if(positions[j].attributes.di1==0 && positions[j].attributes.motion==false)
                    {
                      $rootScope.conpblm++;
                      $rootScope.didnotrun++;
                ...
                if($.inArray(vehicles[i], 'position'))
                {  
                    ...
                    {  
                      $scope.firsttime=1;
                      $scope.tempMarker={label:vehicles[i].name.substr(0, 15), angle:vehicles[i].position.course, title:vehicles[i].name, icon: $scope.imagePath+'/'+$scope.iconFolder+$scope.icon,vehicle:vehicles[i],animation: 'Animation.DROP',latitude:vehicles[i].position.latitude,longitude:vehicles[i].position.longitude,uniqueId:vehicles[i].uniqueId}; 
                    }
                    else
                    {
                      $scope.tempMarker={label:vehicles[i].name.substr(0, 15), angle:vehicles[i].position.course, title:vehicles[i].name, icon: $scope.imagePath+'/'+$scope.iconFolder+$scope.icon,vehicle:vehicles[i],animation: '',latitude:vehicles[i].position.latitude,longitude:vehicles[i].position.longitude,uniqueId:vehicles[i].uniqueId}; 
                    }  
                    $scope.onlineMarkers.push($scope.tempMarker); 
                }    
                /**************************************************************************/
              ...
              //var lat =vehicles[0].position.latitude;
              //var lng = vehicles[0].position.longitude;
              var lat = (latmax+latmin)/2;
              var lng = (longmax+longmin)/2;
              ...
              $('.cmarkerlabels').each(function(i, obj) {
                    //alert($(this).attr('title'));
                    //$(this).trigger('click');
                    //$(this).fadeOut( "slow");
                    $(this).remove();
              });    
            $rootScope.allVehiclespositonsData={group:{id:$scope.get_org_ID(), name:$scope.get_org_name(),vehicles:vehicles,online:$rootScope.online}};
          }
          hideTabLoader();
        }
      })
  ...
  function startTimer() {
      //$scope.update_vehicles_status();
      myInterval = window.setInterval(timerHandler,mytimervalue);
      mytimervalue=30000;
   }
  function stopTimer() {
    window.clearInterval(myInterval);

...
//zonePolygon.setMap(null);
  ...
  $scope.refreshNGMap=function () {
      ...
      /*google.maps.event.addListener(vm.map,'mousemove',function(event) {
        ...
        vm.event = event.latLng.lat()+','+event.latLng.lng();

        $scope.tooltipLatLng=true;
  ...
  $scope.NGnavigateToLocation = function(){  
    alert("sdfsd")
  }
  ...
  $rootScope.$on("showHistoryEventInfowindow", function(event,args){
   ...
   /* var iw = vm.map.infoWindows.foo-ev;

    iw.setPosition(new google.maps.LatLng(args.historyEvent.event_latitude,args.historyEvent.event_longitude));

    vm.map.showInfoWindow('foo-ev');
